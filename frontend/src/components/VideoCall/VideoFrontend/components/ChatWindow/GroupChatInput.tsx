...
export default function ChatInput({ conversation, isChatWindowOpen, recipients }: ChatInputProps) {
  ...
  const handleReturnKeyPress = (event: React.KeyboardEvent) => {
    if (!isMobile && event.key === 'Enter' && !event.shiftKey) {
      ...
      handleSendGroupMessage(messageBody, recipients);
    }
  };

  //TODO Implement handleReturnKeyPress based on our front-end implementation
  /*
  const handleSendMessage = (message: string) => {
    if (isValidMessage) {
      // TODO UNCOMMENT BELOW FOR OFFICIAL VERSION - Brian
      // conversation.sendMessage(message.trim());
      // conversation.sendDirectMessage(message.trim(), 'test123');
      // conversation.sendGroupMessage(message.trim(), ['test123', 'test456']);
      setMessageBody('');
    }
  }; */

  /*
  const handleSendDirectMessage = (message: string, recipient: Player[]) => {
    if (isValidMessage) {
      conversation.sendDirectMessage(message.trim(), recipients[0].userName);
      setMessageBody('');
    }
  };
  */
  ...
  const handleSendGroupMessage = (message: string, recipients: string[]) => {
    if (isValidMessage) {
      conversation.sendGroupMessage(message.trim(), recipients);
      setMessageBody('');
    }
  };
